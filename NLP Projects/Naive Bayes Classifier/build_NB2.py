...
#word_counts_per_label = {}#Counter() #This will help us calculate the probability of each feature in a minute
...
for vector in training_data:
    ...
    pairs = set((pair.split(':')[0],pair.split(':')[1]) for pair in vector[1:])
    for pair in pairs:
        vector_counts.update([pair[0] for x in range(int(pair[1]))])
    if vector[0] not in word_counts_per_label:
        word_counts_per_label[vector[0]] = Counter()
    word_counts_per_label[vector[0]] += vector_counts
    training_vectors.append((vector[0],vector_counts))
...
for vector in test_data:
    ...
    for pair in pairs:
        vector_counts.update([pair[0] for x in range(int(pair[1]))])
    test_vectors.append((vector[0],vector_counts))
    test_labels.append(vector[0])
    
vocab = set()
#test_features = set()
for vector in training_vectors:
    vocab.update(vector[1].keys())
...
for word in vocab:
    ...
    for label in word_counts_per_label:
        word_probs_per_label[word][label] = math.log10((word_counts_per_label[label][word] + cond_prob_delta) /
                                                          (sum_per_label[label] + vocab_length * cond_prob_delta))
    ...
    for word in vocab:
        '''if word in document[1]:
            for label in word_probs_per_label[word]:
                log_classifications[label] += (word_probs_per_label[word][label] * document[1][word])'''
        for label in word_probs_per_label[word]:
            log_classifications[label] += (word_probs_per_label[word][label] * document[1][word])
